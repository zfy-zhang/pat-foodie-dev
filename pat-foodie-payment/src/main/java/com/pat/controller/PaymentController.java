...
import com.pat.resource.AliPayResource;
import com.pat.resource.WXPayResource;
...
public class PaymentController {
	...
	public ResJSONResult createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isBlank(merchantOrderId)) {
			return ResJSONResult.errorMsg("参数[orderId]不能为空");
		}
		if (StringUtils.isBlank(merchantUserId)) {
			return ResJSONResult.errorMsg("参数[userId]不能为空");
		}
		if (amount == null || amount < 1) {
			return ResJSONResult.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
		}
		if (payMethod == null) {
			return ResJSONResult.errorMsg("参数[payMethod]不能为空并且不能小于1");
		}
		if (payMethod != PayMethod.WEIXIN.type && payMethod != PayMethod.ALIPAY.type) {
			return ResJSONResult.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
		}
		if (StringUtils.isBlank(returnUrl)) {
			return ResJSONResult.errorMsg("参数[returnUrl]不能为空");
		}
		...
		try {
			...
			e.printStackTrace();
			ResJSONResult.errorException(e.getMessage());
		}

		if (isSuccess) {
			return ResJSONResult.ok("商户订单创建成功！");
		} else {
			return ResJSONResult.errorMsg("商户订单创建失败，请重试...");
		}
	}
	...
	public ResJSONResult getPaymentCenterOrderInfo(String merchantOrderId, String merchantUserId) {

		if (StringUtils.isBlank(merchantOrderId) || StringUtils.isBlank(merchantUserId)) {
			return ResJSONResult.errorMsg("查询参数不能为空！");
		}

		Orders orderInfo = paymentOrderService.queryOrderInfo(merchantUserId, merchantOrderId);

		return ResJSONResult.ok(orderInfo);
	}
...
//	public ResJSONResult goAlipay(String merchantOrderId, String merchantUserId) throws Exception{
//
//		// 查询订单详情
//		Orders waitPayOrder = paymentOrderService.queryOrderByStatus(merchantUserId, merchantOrderId, PaymentStatus.WAIT_PAY.type);
//
...
//
//		return ResJSONResult.ok(alipayForm);
//	}
